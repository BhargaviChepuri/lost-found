...
public class UserService {
	...
	/**
	 * Method to process a request for claiming an item.
	 * This method checks if the user and the item exist, verifies if the item is already claimed or in progress,
	 * and updates the item request status accordingly. It also saves claim history and returns appropriate responses.
	 *
	 * @param itemRequest the request details for the item
	 * @return ResponseEntity containing a response message with the result of the operation
	 */
	@Transactional
	public ResponseEntity<ResponseMessage> requestItem(ItemsRequest itemRequest) {
		LOGGER.info("Initiating requestItem method with itemRequest: {}", itemRequest);

		Optional<User> userOpt = userRepository.findById(itemRequest.getUser().getUserId());
		if (userOpt.isEmpty()) {
			LOGGER.warn("User not found with ID: {}", itemRequest.getUser().getUserId());
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseMessage(
					"User not found with ID: " + itemRequest.getUser().getUserId(), ItemStatus.UNCLAIMED.toString()));
		}
		...
		if (itemOpt.isEmpty()) {
			LOGGER.warn("Item not found with ID: {}", itemRequest.getItem().getItemId());
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseMessage(
					"Item not found with ID: " + itemRequest.getItem().getItemId(), ItemStatus.UNCLAIMED.toString()));
		}
		Items item = itemOpt.get();
		LOGGER.info("Item found: {}", item);

		Optional<ItemsRequest> existingRequest = itemRequestRepo.findFirstByItem_ItemIdAndStatus(item.getItemId(),
				ItemStatus.CLAIMED);
		if (existingRequest.isPresent()) {
			LOGGER.info("Item has already been claimed by another user");
			return ResponseEntity.ok(new ResponseMessage("Item has already been claimed by another user",
					ItemStatus.CLAIMED.toString()));
		}

		Optional<ItemsRequest> inProgressRequest = itemRequestRepo.findFirstByItem_ItemIdAndStatus(item.getItemId(),
				ItemStatus.PENDING_PICKUP);
		if (inProgressRequest.isPresent()) {
			ItemsRequest request = inProgressRequest.get();
			if (request.getUser().getUserId() == user.getUserId()) {
				LOGGER.info("Claim request found in progress for user {}. Updating status to CLAIMED.",
						user.getUserId());
				request.setStatus(ItemStatus.CLAIMED);
				itemRequestRepo.save(request);

				saveClaimHistory(item, user, ItemStatus.CLAIMED, "Claim approved by admin");

				LOGGER.info("Item successfully claimed by the user: {}", user.getUserId());
				return ResponseEntity.ok(
						new ResponseMessage("Item successfully claimed by the user!", ItemStatus.CLAIMED.toString()));
			} else {
				LOGGER.info("Another user has already initiated a claim for this item");
				return ResponseEntity.ok(new ResponseMessage("Another user has already initiated a claim for this item",
						ItemStatus.PENDING_PICKUP.toString()));
			}
		}

		itemRequest.setUser(user);
		itemRequest.setItem(item);
		itemRequest.setStatus(ItemStatus.PENDING_PICKUP);
		itemRequest.setClaimedDate(LocalDateTime.now());
		itemRequestRepo.save(itemRequest);

		LOGGER.info("Item request initiated for item: {} by user: {}", item.getItemId(), user.getUserId());
		saveClaimHistory(item, user, ItemStatus.PENDING_PICKUP, "Request initiated by user");

		LOGGER.info("Item request successfully created with status: {}", ItemStatus.PENDING_PICKUP);
		return ResponseEntity.status(HttpStatus.CREATED)
				.body(new ResponseMessage("Item request initiated successfully with status PENDING_PICKUP!",
						ItemStatus.PENDING_PICKUP.toString()));
	}
	
	/**
	 * Method to save the claim history for an item and a user.
	 * This records the claim status, claim date, and user details into the claim history for auditing and tracking purposes.
	 *
	...
	/**
	 * Method to fetch details of all items with the status 'OPEN'.
	 * It retrieves all items with the specified status and returns the count and details in the response.
	 *
	...
	/**
	 * Method to fetch all item request details.
	 * This retrieves all item requests regardless of their status and returns the details.
	 *
	...
	public List<ItemsRequest> getItemRequestDetails() {
		...
		res.put(Constants.SUCCESS, false);
		res.put(Constants.MESSAGE, Constants.INVALID_INPUTS);
		LOGGER.info("Fetched {} item request(s).");

		return itemRequestRepo.findAll();

	}
	/**
	 * Method to fetch item request details for a specific user identified by their user ID.
	 * This retrieves all requests made by the user, filtering by their unique user ID.
	 *
	...
	public List<ItemsRequest> getItemRequestDetailsByUserId(int userId) {
		...
		res.put(Constants.SUCCESS, false);
		res.put(Constants.MESSAGE, Constants.INVALID_INPUTS);
		LOGGER.info("Fetched {} item request(s) for user ID: {}", userId);

		return itemRequestRepo.findByUser_UserId(userId);
	}
	/**
	 * Method to fetch claim history and item request details based on the user's email.
	 * It retrieves the claim history and item requests for a user identified by their email address.
	 *
	 * @param email the email of the user for whom to fetch claim history and item requests
	 ...
	 * If the query is empty, it fetches all items; otherwise, it performs a refined search based on item name, color, and category.
	 *
	...
	/**
	 * Method to clean the search query by removing common stop words.
	 * It removes words like 'I', 'my', 'lost', etc., and returns the cleaned query.
	 *
	...
	/**
	 * Method to fetch all items from the repository and convert them into a list of DTOs.
	 * It includes item details such as category, color, status, and user details.
	 *
	...
	private List<ItemsSearchDTO> getAllItems() {
		...
		return itemsRepository.findAll().stream().map(item -> {
			...
			return new ItemsSearchDTO(item.getItemId(), item.getReceivedDate(), item.getExpirationDate(),
					item.getDominantColor(), item.getDetectedText(), item.getOrgId(), item.getDescription(),
					item.getTitle(), item.getItemName(), item.getStatus(),
					item.getUser() != null ? item.getUser().getUserId() : null, item.getImage(),
					item.getUser() != null ? item.getUser().getName() : null,
					item.getUser() != null ? item.getUser().getEmail() : null, categoryName, item.getLatitude(),
					item.getLongitude());
		}).collect(Collectors.toList());
	}
	/**
	 * Method to search for items by item name, color, and category based on a cleaned query.
	 * It processes each term in the query to check if it corresponds to an item name, color, or category.
	 *
	...
	/**
	 * Method to check if a term corresponds to a valid category.
	 * It verifies if the term exists as a category in the repository.
	 *
	...
	private boolean isCategory(String term) {
		...
		boolean exists = categoriesRepo.existsByNameIgnoreCase(term);
		LOGGER.info("Category exists: " + exists);
		return exists;
	}
	/**
	 * Method to check if a term corresponds to a valid color.
	 * It attempts to map the term to a color field from the Color class.
	 *
	 * @param term the term to be checked
	...
	/**
	 * Method to upload an image for search purposes.
	 * It validates the uploaded file, checks its type, and attempts to find matching items based on the uploaded image.
	 *
	...
	/**
	 * Updates the status of a claim and sends a notification to both the user and admin.
	 * 1. Retrieves the claim history by the provided claimId.
	 * 2. Checks if the claim exists in the repository; if not, it throws a ResourceNotFoundException.
	 * 3. Logs the old claim status.
	 * 4. Updates the claim status to the new one provided in the method argument.
	 * 5. Saves the updated claim status back to the repository.
	 * 6. Sends a notification to both the user and admin about the status change.
	 * 7. Returns a response map containing the updated status and a message.
	 *
	 ...
	 * @throws ResourceNotFoundException if the claim with the given claimId is not found.
	 */
	...
	/**
	 * Calculates the statistics for each item category in terms of total weight and item count.
	 *
	 * 1. Initializes maps to store the total weight and item count for each category.
	 * 2. Fetches all categories from the repository.
	 * 3. Initializes category data with zero values for weight and item count.
	 * 4. Retrieves all unclaimed items and processes their category and weight data.
	 * 5. Aggregates the total weight and item count for each category.
	 * 6. Formats the results into a list of maps containing category name, total weight, and total item count.
	 * 7. Returns a response map with the calculated category statistics.
	 *
	...
	/**
	 * Submits a claim request and saves it to the repository.
	 * 1. Logs the starting information for the claim request submission.
	 * 2. Creates a new `ClaimRequest` object and sets its properties.
	 * 3. Saves the claim request to the repository.
