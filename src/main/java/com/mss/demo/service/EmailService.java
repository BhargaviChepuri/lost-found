...
public class EmailService {
	...
	public void sendClaimConfirmationEmail(String userEmail, Items item) {
		...
		String message = buildEmailTemplate(
				"Dear " + item.getUser().getName() + ",\n\nYour claim for item " + item.getTitle()
						+ " has been successful. " + ".",
...
// Send claim notification to admin
	public void sendClaimNotificationToAdmin(Items item) {
		...
		SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
		String formattedExpirationDate = dateFormat.format(item.getExpirationDate());
		String message = "The item <strong>" + item.getTitle() + "</strong> has been claimed by " + "<strong>"
	...
	private String buildEmailTemplateForClaimNotification(String message, ItemStatus status, String expirationDate, Date receivedDate) {
		return "<!DOCTYPE html>" + "<html lang=\"en\">" + "<head>" + "<meta charset=\"UTF-8\">"
				+ "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">" + "<style>"
				+ "body { margin: 0; padding: 0; }" + ".email-container {" + "    width: 100%;"
				+ "    max-width: 600px;" + "    margin: 0 auto;" + "    background-color: #ffffff;"
